...
class SimpleLinearRegression():
    ...
    def fit(self, method: str = None):
        ...
        if method == "beta_estimations":
            ...
            raise ValueError(f"Unknown method '{self.method}' for SimpleLinearRegression.")
    ...
    def _fit_beta_estimations(self):
        ...
        self.beta_1_hat = np.sum( (self.x - x_bar) * (self.y - y_bar) ) / np.sum( (self.x - x_bar)**2 )
    ...
    def _calculate_r_squared(self, y_actual, y_pred):
        return 1 - np.sum((y_actual - y_pred)**2) / np.sum((y_actual - np.mean(y_actual))**2)
    ...
    def _coefficient_estimators(self, x: pd.Series, y: pd.Series, n: int, methods: str = None):
            ...
            for method in methods:
                ...
                try:
                    ...
                    # Performance metrics
                    # RMSE: Root Mean Squared Error - Square root of the average of the squared differences between the predicted values and the actual
                    # MAE: Mean Absolute Error - Average absolute difference between the predicted values and the actual
                    # R_squared:
    ...
    def simulate(
        self, data: pd.DataFrame = None, n_samples_list: list = [10, 100, 1000], noise: float = 1.0, seed: int = 42, methods=None
    ) -> pd.DataFrame:
        ...
        if data is not None and 'x' in data and 'y' in data:
            ...
            results.extend(self._coefficient_estimators(x, y, n=x.size, methods=methods))
        ...
        return pd.DataFrame(results).sort_values(['n_samples', 'method', 'duration_seconds'])
